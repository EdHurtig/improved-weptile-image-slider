	...
	<tr>
		...
		if ( count( $instance['slider-images'] ) > 0 ) {
			...
			foreach ( $instance['slider-images'] as $image ) {
				...
				$image_url = get_site_url() . '/wp-content/plugins/' . basename( dirname( __FILE__ ) ) . '/cache/' . str_ireplace( substr( $image_file_name, - 4 ), '-resized-x' . substr( $image_file_name, - 4 ), $image_file_name );
